// app.js — Cashflow Rush v2.6 Mobile Arcade Edition
// © 2025 pezzaliAPP — MIT
(() => {
  ...
  // Keyboard
  window.addEventListener('keydown', e=>{ const k=e.key; if(k==='ArrowLeft') move(-1,0); if(k==='ArrowRight') move(1,0); if(k==='ArrowUp') move(0,-1); if(k==='ArrowDown') move(0,1); if(k==='z'&&(e.ctrlKey||e.metaKey)) undo(); if(k==='r') loadLevel(levelIndex); });

  // D-Pad
  document.querySelectorAll('.touchpad button').forEach(b=>{
    b.addEventListener('click', ()=>{ move(parseInt(b.dataset.dx), parseInt(b.dataset.dy)); });
  });

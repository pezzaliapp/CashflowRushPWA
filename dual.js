...
(()=>{
  ...
  // Canvas: l’engine v3.0.0 usa #game → rinominiamo il canvas visibile
  ...
  function fitCanvas(){
    ...
    if(isMob){
      ...
      const maxW = stage ? (stage.clientWidth - 24) : 720;
      const maxH = Math.min(window.innerHeight - 320, stage ? (stage.clientHeight - 24) : 720);
      const size = Math.max(520, Math.min(maxW, maxH, 720));
      c.style.width = size+'px'; c.style.height = size+'px';
    }
  }

  // PLAY & avvio audio iOS
  let started = false;
  ...
  function tone(f=660,d=0.05){
    ...
    const o=ac.createOscillator(), g=ac.createGain(); o.type='sine'; o.frequency.value=f; g.gain.value=0.05;
    o.connect(g); g.connect(ac.destination); o.start(); setTimeout(()=>{try{o.stop()}catch{};}, d*1000);
  }
  function startUI(ev){
    ...
    // se il motore espone Game.start(), usiamolo (altrimenti parte alla prima freccia)
  ...
  function bindPlay(){
    ...
    ['click','touchstart','pointerup'].forEach(h=>{
      if(b1) b1.addEventListener(h, startUI, {passive:false});
      if(b2) b2.addEventListener(h, startUI, {passive:false});
    });
    if(c){
      ['touchstart','pointerdown','mousedown'].forEach(h=> c.addEventListener(h, ()=>{ if(!started) startUI(); }, {passive:true}));
    }
  }

  // D-pad → invia frecce (il tuo app.js v3.0.0 ascolta le arrow)
  ...
  // Selettore Classic / Dual (memorizzato)
  (()=>{
    ...
    document.getElementById('gameModeSeg').addEventListener('click', e=>{
      ...
      // se il motore espone Game.reload() ricarichiamo, altrimenti l'utente può usare Reset
